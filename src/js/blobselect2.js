...
(function(){
	...
	var flags = {
		...
		blobselectDebug: 'blobselect-debug',
	};
	...
	blobSelect.prototype = {
		...
		init: function(args) {
			...
			if (this.isInitialized()) {
				return this.debug('Already initialized; init() aborted.');
			}
		...
		destroy: function(args) {
			if (!this.isInitialized()) {
				return this.debug('Not initialized; destroy() aborted.');
			}

			// TODO kill observers, etc.
		...
		saveSettings: function(args) {
			...
			this.debug('Parsed settings: ' + settingsString);
		...
		buildData: function() {
			if (!this.hasElement()) {
				return this.debug('Not initialized; buildData() aborted.');
			}
				...
				for (i=0; i<optgroups.length; i++) {
					...
					for (j=0; j<options.length; j++) {
						...
						row.placeholder = (
							...
							(row.label.toLowerCase() === this.$settings.placeholderOption.toLowerCase())
						);
						rows.push(row);
					}
					rows = this.sort(rows);
		...
		buildContainer: function() {
			if (this.container !== null) {
				this.debug('Container already exists; buildContainer() aborted.');
			}
		...
		buildItems: function(force) {
			...
			if (!force) {
				...
				if (
					...
					(_hash(newValues) !== _hash(oldValues))
				) {
					force = true;
				}
			}
			...
			if (force) {
				this.debug('Rebuilding menu items.');
				var newItems = [],
					tabIndex = 1;
				for (i=0; i<this.$items.length; i++) {
			...
			// unnecessary DOM mutations and event binding.
				...
				this.debug('Updating menu items.');
			...
			this.debug('Menu items have been ' + (force ? 'rebuilt' : 'updated') + '.');
		},
		...
		buildSelections: function(force) {
			...
			if (!force) {
				...
				if (
					...
					(_hash(oldValues) !== _hash(newValues))
				) {
					force = true;
				}
			}
			...
			if (force) {
				this.debug('Rebuilding selections.');
				var newItems = [];
			...
			// Nothing to do.
				...
				this.debug('Selections have not changed.');
				return;
			}

			return this.debug('Selections have been ' + (force ? 'rebuilt' : 'updated') + '.');
		},
		...
		open: function() {
			...
			return this.debug('Dropdown opened.');
		},
		...
		close: function() {
			...
			return this.debug('Dropdown closed.');
		},
				...
				if (this.$me.multiple) {
					for (i=0; i<options.length; i++) {
						...
						if (options[i].selected) {
							...
							this.debug('Unselected: "' + options[i].value + '"');
						}
						else {
							options[i].selected = 1;
							this.debug('Selected: "' + options[i].value + '"');
						}
					}
				}
				else {
		...
		filterItems: _debounce(function() {
			...
			if (!matches) {
				...
				me.debug('No items matched the search.');
			}
			else {
				me.debug('Items filtered by search.');
			}
		 ...
		 * @return void Nothing.
		 */
		debug: function(str, warn) {
			if (!this.$settings.debug) {
				return;
			}

			var date = new Date().toISOString().slice(0, 19).replace('T', ' '),
				lead = 'blobSelect[' + date + '] ';
			...
			try {
				...
				console.warn(lead + 'Could not process debug message.');
			}
		},
	...
	document.addEventListener('DOMContentLoaded', function(){
		var selects = document.querySelectorAll('select[data-blobselect], select[data-blobselect-watch], select[data-blobselect-search], select[data-blobselect-placeholder], select[data-blobselect-placeholder-option], select[data-blobselect-order], select[data-blobselect-order-type], select[data-blobselect-debug]');
	...
	function _hash(key, seed) {
		...
		// Make sure we have a string for the $key.
